...
- ⚠️ **Depende de onde o agente está rodando.** Num ambiente Linux remoto não há Node:
  valide por inspeção estática e lógica pura. Mas **quando o agente roda na própria máquina
  Windows do escritório, o Node 22 está disponível** (`node --version`) — aí dá para testar
  de verdade: subir o painel numa porta livre (`iniciarPainel(3999)`) e exercitar as rotas
  com `curl`, carregar módulos com `node -e`, e validar o JS do navegador extraindo o bloco
  `<script>` do HTML servido e passando por `new Function()` (erro de sintaxe ali quebra o
  painel inteiro e **não** aparece num `require`). Sempre cheque antes de assumir.
- ⚠️ Ao testar contra o banco real, **não imprima conteúdo de cliente** no terminal (vai
  para o `log.txt`): use contagens, tamanhos e presença de campos.
...
sistema/
  ...
  config.js            # opcoes de funcionamento editaveis no painel (config.json + defaults)
  ...
  sync.js              # sincronizacao das fichas entre os PCs do escritorio (sync.json)
...
5. **Debounce**: mensagens em sequência são agrupadas num único atendimento
   (`enfileirarMensagem`/`pendentes`) e viram uma só resposta. O prazo vem de
   [config.js](sistema/config.js) (aba "Outras opções"), lido **a cada mensagem**, e tem
   dois modos (`calcularEsperaMs` em [bot.js](sistema/bot.js)):
   - `ultima` (padrão, ~5s): conta da última mensagem — cada mensagem reinicia tudo.
   - `primeira`: conta da **primeira** mensagem do lote (ex.: 5 min), para o cliente
     falante não adiar a resposta indefinidamente.

   ⚠️ O **piso de silêncio** `MIN_SILENCIO_MS` (5s, não editável) vale nos **dois** modos
   e nunca é sobrescrito — por isso o alvo é `max(primeira + espera, agora + 5s)`. Sem
   ele, no modo `primeira` uma mensagem que chega no fim do prazo seria cortada: o bot
   responderia sem tê-la lido. Não "simplifique" para um `setTimeout` fixo.
...
- `HISTORICO_LIMIT` (30), `DEBOUNCE_MS` (5000), `MIDIA_COOLDOWN_MS` (60000) — **agora
  editáveis no painel** ([config.js](sistema/config.js)); o `.env` vale só como padrão
  inicial (precedência: `config.json` > `.env` > default do código).
- `DATAJUD_API_URL`, `PAINEL_PORTA` (3000).
...
**Transporte — caixa-postal, não conexão direta.** Os PCs estão atrás de NAT, em redes
diferentes, e **não podem depender de estar ligados ao mesmo tempo**. Cada um faz PUT do seu
pacote em `sync.php` (no site do próprio escritório, ao lado do `clientes.php` que o
[crm.js](sistema/crm.js) já usa) e GET do pacote do outro quando quiser. O PHP e o passo a
passo de instalação estão em [servidor/](servidor/) — ele é **burro de propósito**: guarda
blobs por máquina, valida o id contra path traversal, grava com `rename()` atômico e exige o
header `X-Sync-Token`.

**Princípio central — "espelho, não fusão".** Cada PC é dono soberano dos seus arquivos;
aplicar um pacote **nunca** sobrescreve dado local. O que vem de fora entra numa seção
marcada da ficha (`<!-- espelho-sync:inicio -->`, **abaixo** do marcador do escritório), que
o bot lê e nunca escreve. Isso elimina conflito *por construção* — não existe arquivo com
dois donos. Fundir de verdade só acontece por ação explícita do usuário ("internalizar",
Fase 4). O perigo nunca foi a fusão; foi a fusão **automática e invisível**.

⚠️ **`lerPerfilMd` e nunca `readMarkdown` ao montar o pacote**
([context.js](sistema/context.js)). `readMarkdown` devolve o arquivo inteiro, incluindo o
bloco espelho recebido; mandá-lo de volta faria o espelho **crescer em espiral** a cada
ciclo. `lerPerfilMd` usa `extrairPerfil`, os mesmos regexes de linha única que
`atualizarMdCliente` já usava (extraídos para não duplicar) — e os campos do espelho levam o
sufixo `(lá)` justamente para não casarem com eles.

⚠️ **O histórico não é mesclado** na tabela `historico`: estouraria `HISTORICO_LIMIT` e faria
o modelo "lembrar" de conversas que não teve. Viaja só como `resumo_historico`, texto puro
sem IA (últimas ~10 falas **do cliente**; as do assistant estão salvas como JSON bruto).

**Nunca trafegam:** chaves do `.env` (não têm categoria), `liberarTodos` (decisão perigosa e
...
- **Outras opções** — [config.js](sistema/config.js): tempo de espera + modo de contagem
  (ver fluxo, item 5), tamanho da janela de histórico e cooldown do aviso de
  vídeo/documento. A tela mostra o efeito em português ("responde 5 minutos depois da
  primeira mensagem") e avisa quando a espera passa de 1 min (o cliente fica sem retorno
  e não há indicador de "digitando"). Os **limites** (`OPCOES` em `config.js`) são
  aplicados no `salvarConfig` — não só na tela, para valer também se editarem o `.json`
...
- **Privacidade/LGPD**: `clientes/*`, `.env`, `db/*.db`, `whitelist.json`,
  `advogados.json`, `personalidade.txt`, `mensagens.json`, `triagem.json`, `config.json`,
  `sync.json`, `instituicoes/escritorio.md`,
